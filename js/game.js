  ...
  if (inputValue === "" || inputValue === "0") {
    ...
    sortCards.addEventListener("click", () => {
        ...
        const bubbleSort = (cardsArray) => {
          let wall = cardsArray.length - 1; //we start the wall at the end of the array
          while (wall > 0) {
              let index = 0;
              while (index < wall) {
                // if (cardsArray[index].number == "J") {cardsArray[index].number == 11};
                // if (cardsArray[index].number == "Q") {cardsArray[index].number == 12};
                // if (cardsArray[index].number == "K") {cardsArray[index].number == 13};
                // if (cardsArray[index].number == "A") {cardsArray[index].number == 14};                
                // compare the adjacent positions, if the right one is bigger, we have to swap
                if (parseInt(cardsArray[index].number) > parseInt(cardsArray[index + 1].number)) { 
